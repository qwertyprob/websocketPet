...
import { type ChatDto, UserRole } from "@/types/chat";
import { useEffect, useRef, useState } from "react";

export default function ChatCard({ chat }: { chat: ChatDto }) {
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        {messages && messages.length > 0 ? (
          messages.map((msg, index) => (
            <div
              className={`flex ${msg.senderRole === 0 ? "justify-end" : "justify-start"}`}
